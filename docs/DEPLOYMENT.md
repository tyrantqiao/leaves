...
本文用于把当前原型部署到 `183.192.47.155`，并通过 `leaves.tyrantqiao.com` 提供访问。当前版本适合个人使用或小范围演示；它仍然使用 JSON 文件保存行程，不包含用户登录和多用户隔离。
...
```text
leaves.tyrantqiao.com  A  183.192.47.155
```
...
原型服务默认只监听 `127.0.0.1:4173`，外部访问由 Caddy 或 Nginx 负责 HTTPS。
...
Caddy 会自动申请和续期 Let's Encrypt 证书。创建 `/etc/caddy/Caddyfile`：
...
确认 DNS 已经解析到 `183.192.47.155`，并且 80/443 已放行后，访问：
