...
- **上下车站选择（双模式）**：登记铁路车次后自动查询——**能定位车次**（带区间输入，或该车次有历史确认区间）→ 显示成功提示（“已查询到车次信息”）并直接展示全部经停站下拉列表让用户选择出发/到达站；**查询不到** → 车站联想下拉（输入即从 3400+ 车站库搜索，中文/拼音/三字码，点击填充）。确认的区间会被记住，下次登记同一车次直达下拉列表。确认后写入正式行程并带上各站真实到发时刻；也可“跳过”仅保留草稿。
